...
class PPTXGenerator:
    ...
    def create_presentation(self, slides_data: Union[List[dict], List], output_path: str) -> str:
        """
    ...
    def _parse_slide_content(self, content: str) -> dict:
        ...
        for line in lines:
            ...
            if line_stripped.startswith('# ') and h1_found:
                logger.warning(f"Multiple H1 headings detected in slide. Subsequent H1 will be converted to H3: {line_stripped[:50]}")
                # H3として扱う
            ...
            if line_stripped.startswith('## ') and (h2_found or h1_found):
                logger.warning(f"Multiple H2 headings detected in slide. Subsequent H2 will be converted to H3: {line_stripped[:50]}")
                # H3として扱う
        ...
        for line in lines:
            ...
            try:
                ...
                if format_info.get('is_heading', False):
                    ...
                    # テキストと書式を適用
                    self.parser.apply_formatting_to_paragraph(p, parsed_text, format_info)
                ...
                # 番号付きリストの場合
                    ...
                    if hasattr(p, '_element'):
                        ...
                        for elem in pPr.findall('.//{http://schemas.openxmlformats.org/drawingml/2006/main}buNone'):
                            pPr.remove(elem)
                        for elem in pPr.findall('.//{http://schemas.openxmlformats.org/drawingml/2006/main}buChar'):
                            pPr.remove(elem)
