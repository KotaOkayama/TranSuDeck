...
async def set_config(config: APIConfig):
    ...
    try:
        ...
        if not config.api_key or not config.api_url:
            raise HTTPException(
                status_code=400, 
                detail="API Key and URL are required"
            )
...
async def get_config_status():
    ...
    logger.info(f"Config status check:")
...
async def get_models():
    ...
    logger.info(f"get_models called - is_configured: {is_configured()}, translator: {translator is not None}")
    ...
    if not translator:
        ...
        if not initialize_services():
            raise HTTPException(
                status_code=500,
                detail="Failed to initialize translator service"
            )
    ...
    try:
        ...
        for model_id in all_models:
            ...
            if is_supported_model(model_id):
                filtered_models.append(ModelInfo(
                    id=model_id,
        ...
        logger.info(f"Returning {len(filtered_models)} filtered models out of {len(all_models)} total")
